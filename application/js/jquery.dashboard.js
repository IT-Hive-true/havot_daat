...
(function($) { // Create closure.
  ...
  $.fn.dashboard = function(options) {
    ...
    var loading = $(opts.loadingHtml).prependTo(dashboard.element);
    ...
    $('#' + dashboard.id + ' .menutrigger').on('click', function() {
    ...
    $('#' + dashboard.id + ' .' + opts.widgetFullScreenClass).on('click',function(e) {        
      // close the menu
    ...
    $('#' + dashboard.id + ' .controls li').on('click',function(e) {        
      // close the menu
      dashboard.log('widgetCloseMenu event thrown for widget ' + widget.id,2);        
    ...
    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetCloseMenu',function(e,o) {
      dashboard.log("Closing menu " + $(this).attr("id"),1);
      o.widget.closeMenu();
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetOpenMenu',function(e,o) {
      dashboard.log("Opening menu " + $(this).attr("id"),1);
      o.widget.openMenu();
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetDelete',function(e,o) {
    ...
    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetRefresh',function(e,o) {
      o.widget.refreshContent();
    });
    
    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetSetTitle',function(event, o) {
      o.widget.setTitle(o.title);
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetClose',function(e,o) {
      dashboard.log("Closing widget " + $(this).attr("id"),1);
      o.widget.closeContent();
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetOpen',function(e,o) {    
      dashboard.log("Opening widget " + $(this).attr("id"),1);
      o.widget.openContent();
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetShow',function() {    
      $(this).find('.' + opts.widgetContentClass).show();
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetHide',function() {    
      $(this).find('.' + opts.widgetContentClass).hide();
    });
    
    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetAddMetadataValue',function(e,o) {
    ...
    $('#' + dashboard.id + ' .' + opts.widgetTitleClass).on('click',function(e) {
    ...
    $('#' + dashboard.id + ' .' + opts.widgetHeaderClass).on('mouseover',function () {
      $(this).find('.' + opts.iconsClass).removeClass("hidden");
    });
        
    $('#' + dashboard.id + ' .' + opts.widgetHeaderClass).on('mouseout', function () {
    ...
    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetOpenFullScreen',function(e,o) {
      o.widget.openFullscreen();
    });

    $('.' + opts.widgetClass).on('widgetCloseFullScreen',function(e,o) {
      o.widget.closeFullscreen();      
    });

    $('#' + dashboard.id + ' .' + opts.widgetClass).on('widgetEdit',function(e,o) {
    ...
    dashboard.element.on('dashboardStateChange', function(){   
    ...
    dashboard.element.on('dashboardCloseLayoutDialog', function(){   
      // close the dialog
      $('#' + layoutOpts.dialogId).dialog('close');
    });

    // FIXME: why doesn't the on construct work in this case
    ...
    $('.' + addOpts.selectCategoryClass).on('click', function(){     
    ...
    dashboard.element.on('addWidgetDialogSelectCategory', function(e, obj){
    ...
    $('.' + addOpts.addWidgetClass).on('click', function(){      
    ...
    $('.' + addOpts.openDialogClass).on('click', function(){
      dashboard.log('dashboardOpenWidgetDialog event thrown',2);        
      dashboard.element.trigger('dashboardOpenWidgetDialog');
      return false;
    });
    
    dashboard.element.on('dashboardCloseWidgetDialog', function(){   
      // close the dialog
      $('#' + addOpts.dialogId).dialog('close');
    });
        
    dashboard.element.on('dashboardOpenWidgetDialog', function(){
